...
func TestAcummAdd(t *testing.T) {
	atom := new(fc.Atom)
	atom.ID = 0

	in := make(chan fc.IPInbound)

	out, err := stdlib.AccumAdd(atom, in)
	if err != nil {
	...
	go func() {
		...
		in <- fc.NewIPIn("5", fc.Port{Name: "input"})
		in <- fc.NewIPIn("5", fc.Port{Name: "input"})
		in <- fc.NewIPIn([]byte{}, fc.Port{Name: "input"})
	...
	assert.False(t, res.GetIndexSelected())
	assert.False(t, res.GetAll())
	assert.Equal(t, string(res.GetData()), "40")
	res = <-out
	assert.Equal(t, res.GetFrom().Name, "output")
	assert.False(t, res.GetIndexSelected())
	assert.False(t, res.GetAll())
	assert.Equal(t, string(res.GetData()), "1040")
}
